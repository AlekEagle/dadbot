                        ...
                        globalBlacklist.getValueByID(msg.channel.id).then(stat => {
                            ...
                            globalBlacklist.getValueByID(msg.author.id).then(stat => {
                                ...
                                if (msg.content.match(/[\s\S]*?\b([i|l|\|]'?’?m|[i|l|\|] am)\b\s?/i)) {
                                    ...
                                    msg.channel.createMessage({ content: `Hi ${msg.content.replace(/[\s\S]*?\b([i|l|\|]'?’?m|[i|l|\|] am)\b\s?/i, '')}, I'm ${msg.channel.guild.members.get(client.user.id).nick ? msg.channel.guild.members.get(client.user.id).nick : 'Dad'}!`, allowedMentions: { roles: false, users: false, everyone: false } }).catch(() => { });
                                }
                            });
                        });
                    });
