  ...
  exec: (client, msg, args) => {
      ...
      return new Promise((resolve, reject) => {
        owners.Owners.findAll().then(usersDB => {
          if (!client.users.get(usersDB[i].id)) {
            ...
            usersArr.push([
              usersDB[i].id,
              { ...client.users.get(usersDB[i].id), admin: usersDB[i].admin }
            ]);
            if (++i < usersDB.length) getData().then(resolve);
            else resolve();
          }
    ...
    getData().then(() => {
      ...
      output.push(
        `NAME${' '.repeat(
          ownerNames
            .map(e => `${e.username}#${e.discriminator}`.length)
            .sort((a, b) => a - b)
            .reverse()[0] - 4
        )}|USERID${' '.repeat(
          ownerNames
      ...
      ownerNames.forEach(e =>
        output.push(
          `${e.username}#${e.discriminator}${' '.repeat(
            ownerNames
              .map(e => `${e.username}#${e.discriminator}`.length)
              .sort((a, b) => a - b)
              .reverse()[0] - `${e.username}#${e.discriminator}`.length
          )}|${e.id}${' '.repeat(
            ownerNames
              .map(e => e.id.length)
              .sort((a, b) => a - b)
              .reverse()[0] - e.id.length
          )}|${users.get(e.id).admin}${' '.repeat(
            users.get(e.id).admin ? 1 : 0
          )}`
        )
      );
