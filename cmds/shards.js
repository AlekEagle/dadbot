    ...
    function getData() {
      return new Promise((resolve, reject) => {
        grafana
          .remoteEval(
            i,
            'JSON.stringify(client.shards.map(s => { return [s.id, { id: s.id, guildCount: s.client.guilds.filter(g => g.shard.id === s.id).length, userCount: s.client.guilds.filter(g => g.shard.id === s.id).map(g => g.memberCount).reduce((a, b) => a + b, 0), status: s.status.toUpperCase(), ping: s.latency }] }))'
          )
          .then(res => {
        ...
        Math.round(
          (100 *
            shards
              ...
              .reduce((a, b) => a + b, 0)) /
            shards.map(e => e.ping).filter(a => typeof a === 'number').length
        ) / 100
      }ms`;
