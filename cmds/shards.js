...
module.exports = {
    ...
    exec: (client, msg, args) => {
        ...
        output.push(shards.map(s => `${' '.repeat(5 - s.options.firstShardID === currentShard ? `>${s.options.firstShardID}`.length : s.options.firstShardID)}${s.options.firstShardID === currentShard ? `>${s.options.firstShardID}` : s.options.firstShardID}|${s.guilds.size}${' '.repeat(7 - s.guilds.size)}|${s.users.size}${' '.repeat(7 - s.users.size)}|${s.shards.get(s.options.firstShardID).status.toUpperCase()}${' '.repeat(12 - s.shards.get(s.options.firstShardID).status.length)}|${s.shards.get(s.options.firstShardID).latency}ms${' '.repeat(9 - `${s.shards.get(s.options.firstShardID).latency}ms`.length)}`))
