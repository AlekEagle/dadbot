          ...
          msg.channel
            .createMessage({
              embed: {
                ...
                fields: values
                  .slice(0 + numPerPage * page, numPerPage + numPerPage * page)
                  .map((v, i, c) => {
                    return {
            ...
            .then(message => {
              ...
              timeout = setTimeout(() => {
                message.channel
                  .createMessage(
                    `${msg.member.mention} the menu was cancelled due to inactivity!`
                  )
                  .then(mesg => {
                ...
                switch (state) {
                  case 'mainmenu':
                    switch (emoji.name) {
                      ...
                      case '🔼':
                        --cursorPos;
                        globalBlacklist.GlobalBlacklist.findAll().then(
                          values => {
                            if (cursorPos < 0) {
                              --page;
                              if (page < 0) {
                                page =
                                  Math.ceil(values.length / numPerPage) - 1;
                              }
                              cursorPos =
                                values.slice(
                                  0 + numPerPage * page,
                                  numPerPage + numPerPage * page
                                ).length - 1;
                            }
                            if (
                              cursorPos >
                      ...
                      case '➕':
                        ...
                        msg.channel
                          .createMessage(
                            'Say the ID of the user, channel, or guild you would like to add to the blacklist.'
                          )
                          .then(messg => {
                            let id, cmds;
                            function handleAddBlacklistItem(messag) {
                              if (
                                messag.author.id !== msg.author.id ||
                                messag.channel.id !== msg.channel.id
                              )
                                return;
                              switch (state) {
                                case 'addblacklistitemid':
                                  id = messag.content.replace(
                                    /^<?#?@?!?(\d+)>? ?$/,
                                    '$1'
                                  );
                                  messag.delete();
                                  messg.edit(
                                    'Great, now I need a comma separated string of all the commands this entry will be blacklisted from!'
                                  );
                                ...
                                case 'addblacklistitemcmds':
                                  ...
                                  messag.delete();
                                  messg.edit(
                                    "Alright, that's all I need! Entry added!"
                                  );
                                  setTimeout(() => {
                                    messg.delete();
                                  }, ms('5sec'));
                                      ...
                                      globalBlacklist.GlobalBlacklist.findAll().then(
                                        values => {
                                          message.edit({
                                            embed: {
                                              title:
                                                'Bad People, Servers, and Channels',
                                              thumbnail: {
                      ...
                      case '🖋':
                        ...
                        msg.channel
                          .createMessage(
                            ...
                            }\`, send me a comma separated string of all the commands this entry will be blacklisted from!`
                          )
                          .then(messg => {
                            let cmds;
