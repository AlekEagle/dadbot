    ...
    function getData() {
      return new Promise((resolve, reject) => {
        grafana
          .remoteEval(
            i,
            'let nums = require("./functions/numbers");JSON.stringify(client.shards.map(s => { return [s.id, { id: s.id, guildCount: s.client.guilds.filter(g => g.shard.id === s.id).length, userCount: s.client.guilds.filter(g => g.shard.id === s.id).map(g => g.memberCount).reduce((a, b) => a + b, 0), status: s.status.toUpperCase(), ping: s.latency, cmdsRan: nums.cmdsRan, msgsRead: nums.msgsRead, responses: nums.responses }] }))'
          )
          .then(res => {
            ...
            grafana
              .remoteEval(
                i,
                'let { cmdsRan, msgsRead, responses } = require("./functions/numbers");JSON.stringify([Number(process.env.NODE_APP_INSTANCE), {cmdsRan, msgsRead, responses}])'
              )
              .then(statsStr => {
