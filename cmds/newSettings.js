...
module.exports = {
    ...
    exec: (client, msg, args) => {
        let menu = new ReactionMenu(client, msg, msg.author.id, {
            embed: {
                title: 'Settings',
                thumbnail: {
        ...
        menu.addEmoji('default', '👤', () => { menu.setState('usersettings'); selection = 0; });
        ...
        menu.addEmoji('serversettings', '⏹', () => { menu.setState('default'); selection = 0; });
        ...
        menu.addEmoji('serversettings', '⏺', (async () => {
            ...
            await settings.updateValue({ id: newSettings.id, flags: newSettings.flags, RNG: newSettings.RNG });
            return;
        }));
        ...
        menu.addEmoji('serversettings', '❗', (async (message, userID) => {
            ...
            async function changePrefix(messag) {
                ...
                if (prefix === client.commandOptions.prefix) {
                    await prefixes.managePrefixes({ action: 'remove', serverID: msg.channel.guild.id });
                } else {
                    await prefixes.managePrefixes({ action: 'add', prefix, serverID: msg.channel.guild.id });
                }
                await newMsg.delete();
                await messag.delete();
        ...
        menu.addEmoji('channelsettings', '🔃', (async (message, userID) => {
            ...
            async function checkChannel(messag) {
                ...
                if (!message.channel.guild.channels.get(chanID)) {
                    ...
                    await messag.delete();
                    setTimeout(() => newMsg.edit('Mention the channel or send the ID in a message to select a new channel!').catch(() => { }), 5000);
                } else {
        ...
        menu.addEmoji('usersettings', '⏹', () => { menu.setState('default'); selection = 0; });
