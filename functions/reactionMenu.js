...
class ReactionMenu {
  constructor(client, origMessage, user, message, reactions, timeout) {
    ...
    if (!this.channel.permissionsOf(client.user.id).has('manageMessages')) {
      this.channel.createMessage(
        "I'm sorry, but I need the permission `MANAGE_MESSAGES` for this to work!"
      );
    ...
    this.channel
      .createMessage(typeof message === 'function' ? message() : message)
      .then(
        message => {
          ...
          this.addReactions(
            this.message,
            this.states.get(this.state).reactions
          ).then(() => {
            this.timeout = setTimeout(() => {
              ...
              this.client.off(
                'messageReactionAdd',
                this.reactionHandlerInstance
              );
              this.message.channel
                .createMessage(
                  `<@${this.user[0]}> menu timed-out due to inactivity!`
                )
                .then(msg => {
  ...
  async handleReaction(emoji, user) {
    ...
    this.timeout = setTimeout(() => {
      ...
      this.message.channel
        .createMessage(`<@${this.user[0]}> menu timed-out due to inactivity!`)
        .then(msg => {
          setTimeout(() => msg.delete(), ms('5s'));
        });
    }, ms(this.timeoutDur));
    ...
    if (
      this.states
        .get(this.state)
        .emojis.has(emoji.id ? `${emoji.name}:${emoji.id}` : emoji.name)
    )
      await this.states
        .get(this.state)
        .emojis.get(emoji.id ? `${emoji.name}:${emoji.id}` : emoji.name)(
        this.message,
        user
      );
  ...
  addState(state, options) {
    ...
    this.states.set(state, {
      ...
      reactions: options ? options.reactions : [],
      emojis: options
        ? options.emojis instanceof Map
          ? emojis
          : typeof options.emojis !== 'undefined'
          ? new Map(emojis)
          : new Map()
        : new Map()
    });
  ...
  addEmoji(state, emoji, action) {
    ...
    this.states
      .get(state)
      .emojis.set(
        typeof emoji === 'object' ? `${emoji.name}:${emoji.id}` : emoji,
        action
      );
    if (
      this.states
  ...
  async setState(state) {
    ...
    if (this.states.get(this.state).message) {
      await this.message.edit(
        typeof this.states.get(this.state).message === 'function'
          ? await this.states.get(this.state).message()
          : this.states.get(this.state).message
      );
    }
  ...
  removeUser(userID) {
    if (this.user.includes(userID))
      this.user = this.user.filter(e => e !== userID);
    return;
  }
