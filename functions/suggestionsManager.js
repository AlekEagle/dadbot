...
async function sendSuggestion(type, message, suggestion) {
  let msg = await message._client.createMessage(Object.values(channels)[type], {
    embed: {
      title: `New ${
        Object.keys(channels)[type].substr(0, 1).toUpperCase() +
        Object.keys(channels)[type].substr(
          1,
          Object.keys(channels)[type].length
        )
      } | #${suggestion.id}`,
      author: {
      ...
      fields: [
        ...
        },
        {
          ...
          value: message.channel.guild
            ...
            : `\`Private Message <${message.channel.id}>\``,
          inline: true
        },
        {
          name: 'Time',
...
module.exports = {
  ...
  async create(type, message, content) {
    ...
    try {
      suggestion = await Suggestions.create({
        ...
        guildID: message.channel.guild ? message.channel.guild.id : null,
        content,
  ...
  async reply(id, message, content) {
    ...
    message._client.createMessage(suggestion.channelID, {
      content: `Hey! Your ${
      ...
      allowedMentions: { repliedUser: !!suggestion.guildID }
    });
    return await suggestion.update({
      replies: suggestion.replies
        ? [
            ...suggestion.replies,
            { replied: message.author.id, content, at: message.createdAt }
          ]
        : [{ replied: message.author.id, content, at: message.createdAt }]
    });
  },
