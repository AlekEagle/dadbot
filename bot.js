...
function nextShard() {
  ...
  function onDBLVote(data) {
    client.getDMChannel(data.user).then(
      (msg) => {
        msg
          .createMessage("Thanks for voting! This helps me a lot!")
          .catch(() => {});
      },
      () => {
  ...
  if (i < nums.shardCount && !env.DEBUG)
    request.post(
      `https://maker.ifttt.com/trigger/process_started/with/key/${u_wut_m8.iftttToken}`,
      {
        json: {
          value1: "Dad Bot",
          value2: i.toString(),
  ...
  client.on("ready", () => {
    if (i < nums.shardCount) {
      prefixes
        .managePrefixes({ action: "refresh", client })
        .then((prefixes) => {
    ...
    if (i < nums.shardCount && !env.DEBUG) {
      request.post(
        `https://maker.ifttt.com/trigger/bot_restarted/with/key/${u_wut_m8.iftttToken}`,
        {
          json: {
            value1: "Dad Bot",
        ...
        () => {
          console.log(
            `Told IFTTT that shard ${client.options.firstShardID} connected`
          );
        }
      );
      ...
      request.post(
        `https://maker.ifttt.com/trigger/bot_reconnected/with/key/${u_wut_m8.iftttToken}`,
        {
          json: {
            value1: "Dad Bot",
            value2: client.options.firstShardID.toString(),
        ...
        () => {
          console.log(
            `Told IFTTT that shard ${client.options.firstShardID} reconnected`
          );
        }
      );
    }
